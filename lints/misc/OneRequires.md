...
If the script uses syntax or functions which are unavailable in earlier versions, using the `#Requires` directive ensures that error messages are informative. Without it, the interpreter will symply show a syntax error. This cannot be checked at runtime, because a syntax error would prevent script execution.
