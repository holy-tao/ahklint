...
- `compile-docs.ahk` — merges each lint's static `meta` with its sibling `.md` prose into one Hugo page per lint under `site/content/lints/<category>/<id>.md`. Generated pages are gitignored; CI regenerates them before `hugo`. Fails if a lint has no `.md` or no `;~ <id>` example. See [DESIGN.md](../DESIGN.md) §9.
